import { Link, useNavigate } from "react-router-dom";
import { ChangeEvent, useContext, useEffect, useState } from "react";
// import { useNavigate } from "react-router-dom";
import { toast, ToastContainer } from "react-toastify";
import { register } from "../api/axios";
import 'react-toastify/dist/ReactToastify.css';
import AuthenticationContext from "../context/Auth/userContext";
import { AuthContext } from "../types/authContext";
import InfoContext from "../context/Data/InfoContext";
import { InfoContextType } from "../types/typeContext";
export default function SignIn() {
    const router = useNavigate();
    const { updateIsLogin, updateAuth, updateUserData } = useContext(AuthenticationContext) as AuthContext;
    const {
        updateUser
    } = useContext(InfoContext) as InfoContextType;

    const [user, setUser] = useState({
        name: "",
        username: "",
        email: "",
        password: "",
        confirmPassword: "",
        isAdmin: false
    });

    const [buttonDisable, setButtonDisable] = useState(false);
    const [loading, setLoading] = useState(false);

    useEffect(() => {
        const { username, email, password } = user;
        const isUsernameValid = username.length > 2;
        const isEmailValid = email.match(/^[a-z0-9]+@[a-z]+\.[a-z]{2,3}$/);
        const isPasswordValid = password.length > 8;

        setButtonDisable(!(isUsernameValid && isEmailValid && isPasswordValid));
    }, [user]);

    const onUserChange = (e: ChangeEvent<HTMLInputElement>) => {
        setUser((prevUser) => ({
            ...prevUser,
            [e.target.name]: e.target.value
        }));
    };


    const onSignup = async (e) => {
        e.preventDefault();
        if (!user.username) {
            toast.info("Make sure to provide user username");
            return;
        }
        if (!user.email) {
            toast.info("Make sure to provide valid email address");
            return;
        }
        if (!user.name) {
            toast.info("Make sure to provide your name");
            return;
        }
        if (user.password !== user.confirmPassword) {
            toast.info("Make sure that you reenter password in confirm password box");
            return;
        }
        const id = toast.info("Creating your Account!");
        try {
            setLoading(true);
            setButtonDisable(true);
            const resp = await register({
                "username": user.username,
                "name": user.name,
                "email": user.email,
                "password1": user.password,
                "password2": user.confirmPassword,
                "AdminAccount": user.isAdmin ? "on" : "",
            });
            toast.success("User Account Created Successfully!");
            // console.log(resp);
            if (resp.data.refresh) {
                toast.success("Login Successfully!");
                updateIsLogin(true);
                updateUser(resp?.data?.user);
                updateAuth({ accessToken: resp?.data?.access, email: resp?.data?.user?.email, roles: resp.data.user.AdminAccount ? [5150] : [], username: resp?.data?.user?.username });
                updateUserData({ email: resp?.data?.user?.email, name: resp?.data?.user?.name, username: resp?.data?.user?.username });
                router("/");
            } else {
                toast.error(resp.data.error);
            }
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
        } catch (error: any) {
            toast.error(error.response.data.message);
            // console.log(error.response.data.message);
        } finally {
            toast.dismiss(id);
            setLoading(false);
            setButtonDisable(false);
        }
    };

    return (
        <>

            <ToastContainer
                position="top-right"
                autoClose={5000}
                hideProgressBar={false}
                newestOnTop={false}
                closeOnClick
                rtl={false}
                pauseOnFocusLoss
                draggable
                pauseOnHover
                theme="colored"
            />
            <main className="auth layout">


                <div className="container">
                    <div className="layout__box">
                        <div className="layout__boxHeader">
                            <div className="layout__boxTitle">
                                <h3>Register</h3>
                            </div>
                        </div>
                        <div className="layout__body">
                            <h2 className="auth__tagline">Find your study partner</h2>

                            <form className="form" action="" method="POST">
                                <input type="hidden" name="csrfmiddlewaretoken" value="wbHO8nIIvzlp8P2yVMewHjG8kUk7K90rJBdVd8hZE9t46cGbRUmggF66gGRUNRdS" />


                                <div className="form__group form__group">
                                    <label htmlFor="room_name">Name</label>
                                    <input type="text" name="name" maxLength={200} required={true} id="id_name"
                                        value={user.name} onChange={(e) => onUserChange(e)}
                                    // fdprocessedid="alj834"
                                    />
                                </div>

                                <div className="form__group form__group">
                                    <label htmlFor="room_name">Username</label>
                                    <input type="text" name="username" maxLength={150} required={true} id="id_username"
                                        value={user.username} onChange={(e) => onUserChange(e)}
                                    // fdprocessedid="1z2gr6"
                                    />
                                </div>

                                <div className="form__group form__group">
                                    <label htmlFor="room_name">Email</label>
                                    <input type="email" name="email" maxLength={254}
                                        // autofocus={false} 
                                        // fdprocessedid="fwy06e"
                                        required={true} id="id_email"
                                        value={user.email} onChange={(e) => onUserChange(e)}
                                        style={{
                                            "backgroundImage": "url(`&quot;data:image/png;base64,&quot;`) !important", "backgroundRepeat": "no-repeat;", "backgroundSize": "20px;", "backgroundPosition": "97% center;", "cursor": "auto;"
                                        }} data-temp-mail-org="0" />
                                </div>

                                <div className="form__group form__group">
                                    <label htmlFor="room_name">Password</label>
                                    <input type="password" name="password" autoComplete={"new-password"} required={true} id="id_password1"
                                        value={user.password} onChange={(e) => onUserChange(e)}
                                    // fdprocessedid="m7178f"
                                    />
                                </div>

                                <div className="form__group form__group">
                                    <label htmlFor="room_name">Password confirmation</label>
                                    <input type="password" name="confirmPassword" autoComplete={"new-password"} required={true} id="id_password2"
                                        value={user.confirmPassword} onChange={(e) => onUserChange(e)}
                                    // fdprocessedid="i0pbqhf"
                                    />
                                </div>
                                <div className="form__group form__group">
                                    <label htmlFor="room_name">Create as Admin Account</label>
                                    <input type="checkbox" name="isAdmin" id="id_password2"
                                        checked={user.isAdmin}
                                        onChange={() => setUser({ ...user, isAdmin: (!user.isAdmin) })}
                                        defaultChecked={user.isAdmin}
                                    // fdprocessedid="i0pbqhf"
                                    />
                                </div>



                                <button className="btn btn--main" type="submit"
                                    // fdprocessedid="dztium"
                                    onClick={(e) => onSignup(e)}
                                    disabled={buttonDisable}
                                >
                                    <svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32">
                                        <title>lock</title>
                                        <path d="M27 12h-1v-2c0-5.514-4.486-10-10-10s-10 4.486-10 10v2h-1c-0.553 0-1 0.447-1 1v18c0 0.553 0.447 1 1 1h22c0.553 0 1-0.447 1-1v-18c0-0.553-0.447-1-1-1zM8 10c0-4.411 3.589-8 8-8s8 3.589 8 8v2h-16v-2zM26 30h-20v-16h20v16z">
                                        </path>
                                        <path d="M15 21.694v4.306h2v-4.306c0.587-0.348 1-0.961 1-1.694 0-1.105-0.895-2-2-2s-2 0.895-2 2c0 0.732 0.413 1.345 1 1.694z">
                                        </path>
                                    </svg>
                                    Register
                                </button>
                            </form>

                            <div className="auth__action">
                                <p>Already signed up yet?</p>
                                <Link to="/login" className="btn btn--link">Sign Up</Link>
                            </div>
                        </div>
                    </div>
                </div>

            </main >
        </>
    );
}